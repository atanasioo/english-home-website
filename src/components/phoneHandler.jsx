...
function PhoneHandler(props) {
  ...
  const onChangeHandlerAdmin = (e, num) => {
    // console.log("admin")
    // https://www.ishtari.com/motor/v1/index.php?route=account/address/autoComplete&filter_name=76
    // https://www.ishtari.com/motor/v1/index.php?route=account/address/getCustomerByPhone&phone=76113320
    let number = num.replace(/ /g, "");
      ...
      <div>
        {state.admin ? (
          <input
            ...
            className={`border text-d14 font-light ${fromContact && "border focus:border-dborderblack2 text-d16 w-full h-14 mb-2.5 px-5 outline-none rounded-sm"} ${!valid && `${red} !important`}`}
            minLength={7}
            maxLength={15}
